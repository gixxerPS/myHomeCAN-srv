...
/**
 * see parseDataOutCommon(data)
 * @param data {object} - <Buffer 00 00 00 00 00 00 00 40> 
 * @returns {object} - states: <Buffer 00 00 00 00 00 00 00 40> => don't change order, see comment in processimage.js */
ProcMsg.prototype.parseDataIuOut = function parseDataIuOut(data) {
  return {
    iuOut : { states : data}
  }
}
 ...
 * @param data {object} - <Buffer 00 00 00 00 00 00 00 40> 
 * @returns {object} - states: <Buffer 00 00 00 00 00 00 00 40> => don't change order, see comment in processimage.js*/
ProcMsg.prototype.parseDataPuOut = function parseDataPuOut(data) {
  return {
    puOut : {states : data}
  }
}
...
//    txType
    this.onMsgDataClients.forEach(function (client) {
      ...
      case msghlp.uTypes.iu:
        if (2 == idObj.code) { // received output states ?
          client.cb.call(client.obj, idObj, self.parseDataIuOut(data));
        } else if (1 == idObj.code){ // assume inputs
          client.cb.call(client.obj, idObj, self.parseDataIuIn(data));
        }
        break;
      case msghlp.uTypes.pu:
        if (2 == idObj.code) { // received output states ?
          client.cb.call(client.obj, idObj, self.parseDataPuOut(data));
        } else { // no inputs on pu
          log.error('unexpect msg from pu for code: ' + idObj.code);  
        }
        break;
      default:
