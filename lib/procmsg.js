...
ProcMsg.prototype.send = function (id, data) {
    this.sendFcn(id, data);
}
...
ProcMsg.prototype.switchOutput = function (offs, state, rxid) {
  if (!this.staticOutMap[rxid]) {
    if (this.registerOutUnit(rxid)) {
      // TODO error occurred...
    }
  }
  this.staticOutMap[rxid].out[offs] = state;
  return this.staticOutMap[rxid].out;
}

/**
 * @param rxid {string} - unique receiver id
 * @returns {error} - or null if successfully registered */
ProcMsg.prototype.registerOutUnit = function (rxid) {
  if (Object.keys(this.staticOutMap).length > NUM_STATIC_OUT) {
    return new Error('max num output units registered');
  }
  if (this.staticOutMap[rxid]) { // registering twice is ok
    log.debug('already registered out unit. skip: ' + rxid);
    return null;
  }
  this.staticOutMap[rxid] = {
      out   : new Array(64).fill(0)
  };
  log.debug('registered out unit' + rxid);
  return null;
}

ProcMsg.prototype.getOutMap = function () {
  return this.staticOutMap;
}

ProcMsg.prototype.sendAlive = function () {
  this.send(msghlp.ids.alive, 
