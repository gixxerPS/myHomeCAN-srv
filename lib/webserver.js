...
app.get('/debug', function (req, res) {
  res.render(path.join(conf.SERVER.DIR,'views/debug.pug'), 
    ...
    homeconf : homeconf, envvars : process.env} 
  );
});
...
var getInputStates = function (ids) {
  ...
  for (i = 0; i < l; i++) {
    ...
    ins.push( {
      state: ProcImg.getInput(adr.id, adr.offs),
      htmlid : ids[i].htmlid }
    );
  }
  return ins;
...
io.sockets.on('connection', function (socket) {
  ...
  socket.on('ctrlOpt', function (data) {
    //log.debug('ctrlOpt received: ' + util.inspect(data));
    if (data.type === 'pump') {
      logicObj.setPumpOpt(data.id, data.opts);
    } else {
      log.warn('ctrlOpt. unknown type ' + data.type);
    }
  });
  ...
  // get pump info e.g. pressure
  socket.on('pump_req', function (ids) {
    log.debug('requested pump ids:' + util.inspect(ids, {depth:null}));
    io.sockets.emit('pump_res', { pumpInfo : logicObj.getPumpInfo(ids)});
  });
...
io.of('/debug').on('connection', function (socket) {
  ...
  fs.readdir(conf.PATH.LOG, function (err, files) {
    if (err) {
      return;
    }
    // read all .log and not .log.1
    var logfiles = files.filter(name => name.endsWith('.log'));

    // pass all logfiles to client
    logfiles.forEach(file => fs.readFile(
      path.join(conf.PATH.LOG, file), (err, data) => {
      if (err) {
        return;
      }
      socket.emit('logmsg', {fn:file, content:data.toString()});
    }));
  });
