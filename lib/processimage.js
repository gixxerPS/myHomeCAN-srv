 ...
 *      in  : [0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0], // = [in0, ... , in63]
 *      inOld: [0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0] // = [in0, ... , in63]
 *   },
 *   '62' : {
 *      out : [0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0], // = [out63, ... , out0]
 *      in  : [0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0], // = [in0, ... , in63]
...
/**
 * reverse order (outputs) output 0 @ byte 7 bit 7
 * @param {number} offs - 0 .. 12/16
 */
...
/** 
 *  normal order (inputs) input 0 @ byte 0 bit 0
 *  @param offs {number} - 0 .. 12/16*/
var convertOffsIn = function convertOffsIn (offs) {
  var sub = offs/8; // e.g. 1.125
  var subInt = parseInt(sub); // e.g. 1
  return {
    idx: subInt, // e.g. 1
    mask: (1<<(sub - subInt)*8)  // e.g. 0x01
  }
}
...
module.exports = {
    ...
    /**
     * Inputs start at lsb byte 0 and are ordered normal
     * @param offs {number} - zero based offset. iu:0..11 */
    convertOffsIn : convertOffsIn,
...
//          // TODO error occurred...
//        }
        return console.error('setOutput > unknown unit: ' + rxid);
      }
    ...
    /**
     * @param rxid {string} - unique receiver id e.g. '81'
     ...
     * @param offs {number} - zero based offset 0 .. 12/16
     * @returns {number} - 1, 0 */
    getInput : function getInput(rxid, offs) {
      ...
      var convIdx = convertOffsIn(offs);
      if (staticMapIo[key].in[convIdx.idx] & convIdx.mask) {
        return 1;
      } else {
        return 0;
      }
...
//          // TODO error occurred...
//        }
        return console.error('onMsgData > unknown unit: ' + key);
      }
      switch (idObj.txType) {
        case msghlp.uTypes.iu:
