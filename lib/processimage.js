...
if (process.env.MYHOMECANTESTENV) {
  console.error('TEST ENVIRONMENT ACTIVE!!!');
  staticMapAlive = {
      '92':{
         "sw": "0.1.0",
         "cnt": 111,
...
var updateAliveMap = function updateAliveMap(idStr, dataObj) {
...
module.exports = {
    ...
    /**
     * @param rxid {string} - unique receiver id + code; e.g. '92.1'
     * @param offs {number} - zero based offset. pu:0..15; iu:0..11
...
//          // TODO error occurred...
//        }
        return console.error('unknown unit');
      }
    ...
    getOutput : function getOutput(rxid, offs) {
      ...
      var convIdx = convertOffs(offs);
      if (staticMapIo[rxid].out[convIdx.idx] & convIdx.mask) {
        return 1;
      } else {
        return 0;
      }
...
//      mu.cl('[0]=' + data.iuIn.states[0] + ' [1]=' + data.iuIn.states[1])
      ...
      var key = idObj.txStr + '.' + idObj.code.toString();
...
//        }
      ...
      switch (idObj.txType) {
      case msghlp.uTypes.iu:
        staticMapIo[key].in = data.iuIn.states;
        break;
      }
    },
