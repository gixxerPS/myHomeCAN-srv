...
function LogicApp(createMaps) {
  ...
  /* tanks.
   * {fill_addr : {state:0|1, level:x%, empty:false, full:false} }
  */
  this.tankMap = {};
    ...
    for(var room in homeconf[floor]) {
      for(var item in homeconf[floor][room]) {
        ...
        if (iObj.type === 'light' || iObj.type === 'receptacle') {
          ...
          if (!this.tankMap[iObj.fill_addr]) {
            this.tankMap[iObj.fill_addr] = {state:0, level: 0, empty: false, full:false};
          } else {
            // TODO: Warning for duplicated tanks in homeconfig!
          }
        }
      }
    }
...
LogicApp.prototype.getValveMap = function getValveMap(ids) {
  var i, tmpMap = {};
  if (!ids) {
