...
var SHUTTERDELAY = 1000;
if (process.env.MYHOMECANTESTENV) {
  SHUTTERDELAY = 20;
} 

/**
 * Time [ms] to delay switch off pump after pressure and no flow reached. */
var PUMPDELAY = 5000;
if (process.env.MYHOMECANTESTENV) {
  PUMPDELAY = 20;
} 
...
/**
 * Time [ms] to enable pump restart after it was switched off. */
var PUMPINTERLOCK = 30000;
if (process.env.MYHOMECANTESTENV) {
  PUMPINTERLOCK = 20;
} 
        ...
        if (iObj.type === 'light' || iObj.type === 'receptacle') {
          ...
          if (!this.pumpMap[floor+'_'+room+'_'+item]) {
            this.pumpMap[floor+'_'+room+'_'+item] = {
              ...
              interlockRestart : {active:false, 
                tObj:new mu.timer(function(){self.onPumpIlockRestartTimeout(floor+'_'+room+'_'+item)}, PUMPINTERLOCK)},
              opts: {
                autoEnable : false
              }};
...
LogicApp.prototype.updatePumpState = function updatePumpState(id, subObj, state) {
  ...
  log.debug('updatePumpState: ' + id + 
  ' min=' + this.pumpMap[id].min + ' ,pressure='+this.pumpMap[id].pressure +
  ' flow=' + this.pumpMap[id].flow);
...
LogicApp.prototype.exePumpLogic = function exePumpLogic(id) {
  ...
  if (!pObj.opts.autoEnable) {
    ...
    // old auto command on ? => skip manual switch on
    if (ProcImg.getOutput(adr.id, adr.offs) && pObj.state) {
      state = 0;
      pObj.state = state; // 0
      ProcImg.setOutput(adr.id, adr.offs, state);
      pObj.interlockRestart.tObj.start(); // avoid short term restart
      pObj.interlockRestart.active = true;
    }
    return;
  }
  if (pObj.opts.autoEnable) {
    // condition for switch on
    if (pObj.pressure && pObj.min) {
      if (pObj.interlockRestart.active) {
        log.debug('pump: ' + id + ' interlock active => no switch on allowed');
        return;
      }
      log.debug('pump: ' + id + ' pressure needed => start');
      state = 1;
      pObj.state = state; // 1
      ProcImg.setOutput(adr.id, adr.offs, state);
      if (pObj.timeObj) {
    ...
    // condition for switch off ?
    } else if (!pObj.pressure && !pObj.flow) {
      log.debug('pump: ' + id + ' pressure ok & no flow => start timeout');
      // switch off delayed
      if (!pObj.timeObj) {
        pObj.timeObj = new mu.timer(function(){self.onPumpTimeout(id)}, PUMPDELAY);
        pObj.timeObj.start(PUMPDELAY);
      } else {
        pObj.timeObj.start(PUMPDELAY);
      }
    }
...
LogicApp.prototype.onPumpTimeout = function onPumpTimeout(id, adr) {
  var pObj = this.pumpMap[id];
  if (!pObj.pressure && !pObj.flow) {
    log.debug('timeout pump: ' + id + ' pressure ok & no flow => stop');
    var adr = msghlp.str2IdOffs(pObj.out_addr);
    var state = 0;
    pObj.state = state; // 0
    ProcImg.setOutput(adr.id, adr.offs, state);
...
LogicApp.prototype.onPumpIlockRestartTimeout = function onPumpIlockRestartTimeout(id, adr) {
  log.debug('timeout pump interlock: ' + id + ' restart now enabled');
  var pObj = this.pumpMap[id];
  pObj.interlockRestart.active = false;
  this.exePumpLogic(id);
}
